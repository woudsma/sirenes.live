...
// (unix seconds) — the same key the ESP32 names the streamed WAV file after, so
// audio and metadata are linked without a separate id. Stats are aggregated in
// SQL to produce exactly the JSON shapes the on-device firmware serves (see
// firmware/src/events.cpp), so the copied chart components render unchanged.
//
...
// Daytime window for the "longest quiet streak" stat: only 07:00–23:00 local
// counts toward a streak; night hours are skipped (they neither extend nor
// break it). Mirrors DAY_START/DAY_END in the web KpiTiles.
const DAY_START_HOUR = 7
const DAY_END_HOUR = 23

// Daytime seconds within [t1, t2) (unix seconds, t1 < t2), summing each day's
// overlap with that day's local 07:00–23:00 window. Uses local-time Date
// construction so DST transitions are handled by the engine; relies on the
// process TZ being the deployment's local zone (same assumption as the SQL
// 'localtime' bucketing above).
...
// Local YYYY-MM-DD for a unix epoch, matching the SQL date(…, 'localtime')
// bucketing (relies on the process TZ being the deployment's local zone).
...
// Longest run of quiet daytime between two consecutive detections. `epochs` must
// be ascending. Only daytime gaps count, so a multi-day quiet spell accumulates
// 16h per day and skips the nights between. Returns the streak length in seconds
// plus the local dates of the sirens that bound it (null when there's no gap).
function longestQuietDaytimeStreak(epochs) {
  let longest = 0
  let from = null
  let to = null
...
export function createStore(dbPath) {
  ...
  const stmts = {
    ...
    `),
    // Just the timestamps, oldest → newest, for the quiet-streak scan.
