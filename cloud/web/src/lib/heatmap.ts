...
export const HEAT_LEVELS = ['brand.100', 'brand.200', 'brand.300', 'brand.400', 'brand.500', 'brand.700']
...
export function heatColor(count: number, max: number): string {
  ...
  const idx = Math.min(HEAT_LEVELS.length - 1, Math.max(0, Math.ceil(ratio * HEAT_LEVELS.length) - 1))
